...
/* pio2, not pio1.
 *
 * In murmnes this driver owns pio1 because nothing else wants it. Here
 * pio0 carries the inter-processor link and pio1 the I2S audio, both of
 * which are brought up before any gamepad dialog can be opened. pio2 is
 * the one that is free on every board in the fleet. */
static PIO pio = pio2;
static int8_t sm1 = -1;
static int8_t sm2 = -1;
                  ...
                  uint8_t data2Pin, uint8_t latPin)
{
    /* Idempotent.
     *
     * In murmnes this runs once from main() and never again. Here it is
     * called every time the gamepad dialog is opened, and without this
     * guard the second visit claimed two more state machines and added a
     * second copy of the program — so the third visit failed outright,
     * for reasons that would have looked like a hardware fault. */
    if (pad_initialized)
        return true;
