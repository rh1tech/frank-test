...
/* Frames actually clocked out, for ui_video_tv.c. See the increment. */
volatile uint32_t tv_frames_emitted = 0;
...
static bool __time_critical_func(video_timer_callbackTV)(repeating_timer_t* rt) {
    ...
    while (dma_inx_out != dma_inx) {
        ...
        if (line_active == video_mode.N_lines) {
            line_active = 0;
            frame_i++;
            /* Third and last edit to the vendored driver: publish the
             * emitted-frame count. The "Video output" test needs proof
             * the encoder is live, and the only honest source is this —
             * counting how often the *interface* repaints says nothing
             * about whether anything reaches the connector, and reads
             * zero whenever the screen happens to be still. */
            tv_frames_emitted++;
...
void graphics_init() {
    ...
    /* Was: DREQ_PIO1_TX0 + SM, with a special case for pio0 — which
     * predates the RP2350's third PIO and silently hands pio2 the
     * *pio1* request line. Here that is I2S's, so the DMA waited on a
     * DREQ the audio state machine controls and the encoder never got a
     * single word out: PIO stalled on `out pins, 8` with an empty FIFO,
     * DMA frozen with 908 transfers pending, and the line alarm firing
     * happily at 30 kHz the whole time.
     *
     * pio_get_dreq() is the SDK's own accessor and gets all three
     * right. This is the second edit to the vendored driver, after the
     * pio2 selection in tv-software.h, and for the same reason. */
