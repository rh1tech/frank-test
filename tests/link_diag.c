...
/* Ported from frank_core2/firmware/master/src/diag_link.c, which is
 * validated on the hardware. The changes are confined to how it reports:
 * console_log() becomes printf(), and the fixed screen rows it used to
 * write into are gone — the registry owns the display now.
 *
 * The recovery logic is untouched, deliberately. It encodes things that
 * were learned from a board rather than reasoned out: that the three
 * mechanisms recover different failures, that a reset needs a backoff or
 * it becomes a reboot loop, and that a reset test has to see the slave
 * actually stop answering before it has proved anything. */
...
/* One place to prefix and flush, so a link failure is legible in a log
 * that also carries test output. */
...
_Static_assert(SWEEP_COUNT == 4,
               "diag_link_result_t reserves four sweep slots");
...
void diag_link_run(diag_link_result_t *out) {
    ...
    /* ---- Handshake ---- */
    link_log("link: saying hello to slave...");

    /* Retry, and say which step failed.
     *
     * "no response" covers three different faults — the doorbell never
     * went out, the answer never came back, or something other than a
     * HELLO_ACK arrived — and they point at different halves of the
     * link. A single line that cannot tell them apart is how a
     * reconnect probe succeeding and the very next HELLO failing looked
     * like the same problem twice. */
    bool hello_ok = false;
    for (int attempt = 0; attempt < 3 && !hello_ok; attempt++) {
        if (attempt) {
            link_log("hello: retry %d after resync", attempt);
            link_m_resync(&g_session);
            sleep_ms(50);
        }

        if (!link_m_send_ctrl(&g_session, LINK_OP_HELLO, 0, 0, NULL, 0)) {
            link_log("hello: send failed (slave not taking the doorbell)");
            continue;
        }
        if (!link_m_recv_ctrl(&g_session)) {
            link_log("hello: no reply within the handshake timeout");
            continue;
        }
        const uint8_t op = ((const link_hdr_t *)g_ctrl_rx)->op;
        if (op != LINK_OP_HELLO_ACK) {
            link_log("hello: unexpected op 0x%02X (wanted HELLO_ACK)",
                     (unsigned)op);
            continue;
        }
        hello_ok = true;
    ...
    if (!hello_ok) {
        link_log("link: no response from slave after 3 attempts");
        return;
    }
    ...
    link_log("link: slave up, %u MHz, proto ok",
                (unsigned)(out->slave_info.sys_clk_hz / 1000000u));

    /* ---- Remote peripheral self-test ---- */
    link_log("link: requesting slave self-test...");
    if (link_m_send_ctrl(&g_session, LINK_OP_SELFTEST, 0, 0, NULL, 0) &&
        ...
        link_log("link: slave self-test received");
    } else {
        link_log("link: slave self-test timed out");
    }

    /* ---- Latency ---- */
    if (link_m_ping(&g_session, PING_ROUNDS, &out->rtt_ns)) {
        link_log("link: ctrl round-trip %u.%02u us",
        ...
        link_log("link: ping failed - resyncing");
        link_m_resync(&g_session);
    }
    ...
    for (unsigned i = 0; i < SWEEP_COUNT; i++) {
        ...
        if (!set_rate(q88)) {
            link_log("link: slave refused divider %u.%02u",
                        q88 >> 8, ((q88 & 0xFF) * 100) >> 8);
            out->all_passed = false;
            continue;
        }

        link_log("link: %u.%02ux  streaming %u MiB each way...",
                    q88 >> 8, ((q88 & 0xFF) * 100) >> 8,
        ...
        if (link_m_send_ctrl(&g_session, LINK_OP_VERIFY_M2S, VERIFY_BLOCKS, 0, NULL, 0) &&
            ...
            if (vr.byte_errors)
                link_log("link: M->S %u bad bytes in %u KiB",
                            (unsigned)vr.byte_errors,
                            (unsigned)(vr.bytes / 1024u));
        } else {
            ran = false;
            link_log("link: M->S verify did not complete");
            link_m_resync(&g_session);
        }
        ...
        if (link_m_send_ctrl(&g_session, LINK_OP_VERIFY_S2M, VERIFY_BLOCKS, 0, NULL, 0) &&
            ...
            errors += vr.byte_errors + vr.timeouts;
            if (vr.byte_errors)
                link_log("link: S->M %u bad bytes in %u KiB",
                            (unsigned)vr.byte_errors,
                            (unsigned)(vr.bytes / 1024u));
        } else {
            ran = false;
            link_log("link: S->M verify did not complete");
            link_m_resync(&g_session);
        }
...
bool diag_link_reset_slave(void) {
    if (!slave_answers()) {
        link_log("reset: slave not answering to begin with");
        return false;
    }
    link_log("reset: slave alive, pulsing FS for %u ms...",
    ...
    if (!went_away) {
        link_log("reset: slave never stopped answering - FS ignored");
        return false;
    }
    link_log("reset: slave went down");
    ...
    for (int i = 0; i < 100; i++) {
        if (slave_answers()) {
            ...
            link_log("reset: slave back after %u ms", (unsigned)ms);
            return true;
        }
        sleep_ms(100);
    }
    link_log("reset: slave did not come back");
    return false;
}

/* diag_link_render() is gone: the registry owns the display. */
